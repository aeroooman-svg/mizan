...
function CustomTabBar({ state, descriptors, navigation, onAddPress }: CustomTabBarProps) {
  ...
  const renderTab = (routeIndex: number) => {
    ...
    if (route.name === "index") {
      ...
      label = language === 'ar' ? "الرئيسية" : (language === 'ml' || language === 'hi') ? "ഹോം" : "Home";
    } else if (route.name === "transactions") {
      iconName = isFocused ? "list" : "list-outline";
      label = language === 'ar' ? "المعاملات" : (language === 'ml' || language === 'hi') ? "ഇടപാടുകൾ" : "Trans";
    } else if (route.name === "stats") {
      iconName = isFocused ? "pie-chart" : "pie-chart-outline";
      label = language === 'ar' ? "إحصائيات" : (language === 'ml' || language === 'hi') ? "സ്ഥിതിവിവരങ്ങൾ" : "Stats";
    } else if (route.name === "financial-plan") {
      iconName = "flag";
      isMaterial = true;
      label = language === 'ar' ? "الخطة" : (language === 'ml' || language === 'hi') ? "പ്ലാൻ" : "Plan";
    }
    ...
    <View style={styles.floatingBarContainer}>
      ...
      <View style={styles.tabBarInner}>
        ...
        <Pressable
          ...
          accessibilityLabel={language === 'ar' ? 'إضافة معاملة جديدة' : (language === 'ml' || language === 'hi') ? 'പുതിയ ഇടപാട് ചേർക്കുക' : 'Add new transaction'}
        >
          <LinearGradient
        ...
        >
          <View style={styles.actionMenuSheet}>
            ...
            <Text style={styles.actionMenuTitle}>
              {language === 'ar' ? 'إجراءات سريعة' : (language === 'ml' || language === 'hi') ? 'ദ്രുത നടപടികൾ' : 'Quick Actions'}
            </Text>
            ...
            <View style={styles.actionMenuOptions}>
              ...
              >
                ...
                <View style={styles.actionOptionInfo}>
                  <Text style={styles.actionOptionName}>
                    {language === 'ar' ? 'إضافة معاملة جديدة' : (language === 'ml' || language === 'hi') ? 'പുതിയ ഇടപാട് ചേർക്കുക' : 'Add New Transaction'}
                  </Text>
                  <Text style={styles.actionOptionDesc}>
                    {language === 'ar' ? 'سجل مصروفاتك أو مداخيلك اليومية فوراً' : (language === 'ml' || language === 'hi') ? 'വരുമാനവും ചെലവും ഉടൻ രേഖപ്പെടുത്തൂ' : 'Log your daily income or expenses instantly'}
                  </Text>
                </View>
              ...
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel={language === 'ar' ? 'مسح فاتورة بالكاميرا' : (language === 'ml' || language === 'hi') ? 'ബില്ല് സ്കാൻ ചെയ്യുക' : 'Scan Receipt'}
              >
                ...
                <View style={styles.actionOptionInfo}>
                  <Text style={styles.actionOptionName}>
                    {language === 'ar' ? 'مسح فاتورة بالكاميرا' : (language === 'ml' || language === 'hi') ? 'ബില്ല് സ്കാൻ ചെയ്യുക' : 'Scan Receipt'}
                  </Text>
                  <Text style={styles.actionOptionDesc}>
                    {language === 'ar' ? 'التقط صورة للفاتورة واستخرج البيانات تلقائياً' : (language === 'ml' || language === 'hi') ? 'പേപ്പർ ബില്ല് സ്കാൻ ചെയ്ത് ചെലവ് രേഖപ്പെടുത്തൂ' : 'Scan a paper receipt to log expenses automatically'}
                  </Text>
                </View>
                <Ionicons name={language === 'ar' ? "chevron-back" : "chevron-forward"} size={18} color={colors.textTertiary} />
              </Pressable>
              ...
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel={language === 'ar' ? 'إنشاء محفظة جديدة' : (language === 'ml' || language === 'hi') ? 'പുതിയ വാലറ്റ് ഉണ്ടാക്കുക' : 'Create New Wallet'}
              >
                ...
                <View style={styles.actionOptionInfo}>
                  <Text style={styles.actionOptionName}>
                    {language === 'ar' ? 'إنشاء محفظة جديدة' : (language === 'ml' || language === 'hi') ? 'പുതിയ വാലറ്റ് ഉണ്ടാക്കുക' : 'Create New Wallet'}
                  </Text>
                  <Text style={styles.actionOptionDesc}>
                    {language === 'ar' ? 'أضف بطاقة أو محفظة كاش جديدة لتتبعها' : (language === 'ml' || language === 'hi') ? 'പുതിയ ബാങ്ക് കാർഡോ വാലറ്റോ ചേർക്കൂ' : 'Add a new bank card or cash wallet'}
                  </Text>
                </View>
                <Ionicons name={language === 'ar' ? "chevron-back" : "chevron-forward"} size={18} color={colors.textTertiary} />
              </Pressable>
            ...
            >
              <Text style={styles.actionMenuCloseText}>
                {language === 'ar' ? 'إغلاق' : (language === 'ml' || language === 'hi') ? 'അടയ്ക്കുക' : 'Close'}
              </Text>
            </Pressable>
          </View>
        </Pressable>
