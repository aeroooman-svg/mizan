...
import getAddTransactionStyles from '@/components/transactions/addTransactionStyles';
import TransactionCalculatorModal from '@/components/transactions/TransactionCalculatorModal';
import CustomCategoryModal from '@/components/transactions/CustomCategoryModal';
...
export default function AddTransactionScreen() {
  ...
  const styles = useMemo(() => getAddTransactionStyles(colors), [colors]);
  ...
  const handleSaveCustomCategory = async (nameAr: string, nameEn: string, icon: string, color: string) => {
    try {
      if (editingCategory) {
        ...
        const newCat = await addCustomCategory(nameAr, nameEn, icon, color, type === 'transfer' ? 'expense' : type);
  ...
  return (
    ...
    >
      ...
      <TransactionCalculatorModal
        ...
        onConfirm={(amt) => setAmount(amt)}
        title={t.calculator}
        saveText={t.save}
        primaryColor={selectedWallet?.color}
        colors={colors}
