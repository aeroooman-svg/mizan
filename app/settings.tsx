...
export default function SettingsScreen() {
  ...
  const handleTestNotification = async () => {
    ...
    await sendImmediateNotification(
      loc('🔔 تجربة تنبيه ميزان المالي', '🔔 Mizan Notification Test', '🔔 മീസാൻ അറിയിപ്പ് ടെസ്റ്റ്'),
      loc(
        ...
        'സ്മാർട്ട് അറിയിപ്പ് സംവിധാനം ശരിയായി പ്രവർത്തിക്കുന്നു! ബജറ്റ് മുന്നറിയിപ്പുകൾ കൃത്യസമയത്ത് ലഭിക്കും.'
      )
    );
    Alert.alert(
      loc('تم الإرسال 🚀', 'Sent 🚀', 'അയച്ചു 🚀'),
      loc('تم إرسال إشعار تجريبي إلى جهازك بنجاح.', 'A test notification has been dispatched to your device.', 'ടെസ്റ്റ് അറിയിപ്പ് നിങ്ങളുടെ ഫോണിലേക്ക് വിജയകരമായി അയച്ചു.')
    );
  };
  ...
  const handleSync = async () => {
    ...
    if (result) {
      ...
      Alert.alert(
        loc('تمت المزامنة', 'Synced', 'സിങ്ക് ചെയ്തു'),
        loc('تمت مزامنة البيانات السحابية بنجاح!', 'Cloud data synced successfully!', 'ക്ലൗഡ് ഡാറ്റ വിജയകരമായി സിങ്ക് ചെയ്തു!')
      );
    } else {
      safeHaptic.notification(Haptics.NotificationFeedbackType.Error);
      Alert.alert(
        loc('خطأ', 'Error', 'പിശക്'),
        loc('فشلت المزامنة، يرجى التحقق من اتصالك بالإنترنت', 'Sync failed, please check your connection', 'സിങ്ക് പരാജയപ്പെട്ടു, ഇന്റർനെറ്റ് കണക്ഷൻ പരിശോധിക്കുക')
      );
    }
  };
  ...
  const handleLogout = () => {
    ...
    setConfirmModalState({
      visible: true,
      title: loc('تسجيل الخروج', 'Logout', 'ലോഗ് ഔട്ട്'),
      message: loc('هل أنت متأكد من رغبتك في تسجيل الخروج؟', 'Are you sure you want to logout?', 'ലോഗ് ഔട്ട് ചെയ്യണമെന്ന് ഉറപ്പാണോ?'),
      confirmText: loc('تسجيل الخروج', 'Logout', 'ലോഗ് ഔട്ട്'),
      isDestructive: true,
      onConfirm: async () => {
  ...
  const handleTogglePin = (value: boolean) => {
    ...
    if (value) {
      ...
      setConfirmModalState({
        visible: true,
        title: loc('تعطيل رمز PIN', 'Disable PIN', 'പിൻ ലോക്ക് ഒഴിവാക്കുക'),
        message: loc('هل أنت متأكد من رغبتك في إيقاف قفل رمز PIN؟', 'Are you sure you want to disable PIN lock?', 'പിൻ ലോക്ക് ഒഴിവാക്കണമെന്ന് ഉറപ്പാണോ?'),
        confirmText: loc('تعطيل', 'Disable', 'ഒഴിവാക്കുക'),
        isDestructive: true,
        onConfirm: async () => {
          setConfirmModalState(prev => ({ ...prev, visible: false }));
          await disablePin();
  ...
  const handleToggleBiometrics = async (value: boolean) => {
    ...
    if (value) {
      if (!isPinEnabled) {
        Alert.alert(
          loc('تنبيه الأمان', 'Security Notice', 'സുരക്ഷാ മുന്നറിയിപ്പ്'),
          loc(
            ...
            'ഫേസ് ഐഡി ഓണാക്കുന്നതിന് മുൻപ് ബാക്കപ്പായി പിൻ ലോക്ക് സജ്ജമാക്കുക.'
          )
        );
        return;
      }
      if (Platform.OS === 'web') {
        Alert.alert(
          loc('تنبيه Face ID', 'Face ID Notice', 'ഫേസ് ഐഡി മുന്നറിയിപ്പ്'),
          loc(
            ...
            'ഈ ഫോണിൽ ഫേസ് ഐഡി പിന്തുണയ്ക്കുന്നില്ല.'
          )
        );
        return;
      }
      ...
      if (!ok) {
        Alert.alert(
          ...
          loc(
            ...
            'ഫേസ് ഐഡി ഓണാക്കാൻ കഴിഞ്ഞില്ല. ഫോൺ സെറ്റിങ്സിൽ കോൺഫിഗർ ചെയ്തിട്ടുണ്ടെന്ന് ഉറപ്പാക്കുക.'
          )
        );
      } else {
        safeHaptic.notification(Haptics.NotificationFeedbackType.Success);
      ...
      if (confirmedPin.length < 4) {
        ...
        if (next.length === 4) {
          if (next === enteredPin) {
            ...
            setPinError(loc('الرمز غير متطابق، يرجى المحاولة ثانية', 'PINs do not match, try again', 'പിൻ ഒത്തുപോകുന്നില്ല, വീണ്ടും ശ്രമിക്കുക'));
            safeHaptic.notification(Haptics.NotificationFeedbackType.Error);
            setConfirmedPin('');
          }
        }
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      Alert.alert(loc('خطأ', 'Error', 'പിശക്'), loc('فشل إنشاء النسخة الاحتياطية', 'Failed to create backup', 'ബാക്കപ്പ് നിർമ്മാണം പരാജയപ്പെട്ടു'));
    }
  };

  const handlePerformRestore = async () => {
    if (!restoreJsonInput.trim()) {
      Alert.alert(loc('تنبيه', 'Alert', 'മുന്നറിയിപ്പ്'), loc('يرجى لصق نص كود النسخة الاحتياطية (JSON)', 'Please paste backup JSON content', 'ബാക്കപ്പ് JSON ഇവിടെ പേസ്റ്റ് ചെയ്യുക'));
      return;
    }
    setIsRestoring(true);
    ...
    try {
      ...
      if (ok) {
        ...
        setRestoreJsonInput('');
        Alert.alert(
          loc('تمت الاستعادة بنجاح 🎉', 'Restore Successful 🎉', 'വിജയകരമായി പുനഃസ്ഥാപിച്ചു 🎉'),
          loc('تم استرجاع كامل معاملاتك ومحافظك وخططك بنجاح!', 'All transactions, wallets, and plans restored successfully!', 'എല്ലാ ഇടപാടുകളും വാലറ്റുകളും പ്ലാനുകളും പുനഃസ്ഥാപിച്ചു!')
        );
      } else {
        safeHaptic.notification(Haptics.NotificationFeedbackType.Error);
        Alert.alert(loc('خطأ', 'Error', 'പിശക്'), loc('ملف النسخة الاحتياطية غير صالح أو تالف', 'Invalid backup JSON file', 'അസാധുവായ ബാക്കപ്പ് ഫയൽ'));
      }
    } catch (e) {
      Alert.alert(loc('خطأ', 'Error', 'പിശക്'), loc('حدث خطأ أثناء استعادة البيانات', 'Failed to restore backup', 'ഡാറ്റ പുനഃസ്ഥാപിക്കുന്നതിൽ പിശക് സംഭവിച്ചു'));
    } finally {
  ...
  const handleClearAllData = () => {
    ...
    setConfirmModalState({
      visible: true,
      title: loc('مسح جميع البيانات', 'Clear All Data', 'എല്ലാ വിവരങ്ങളും മായ്ക്കുക'),
      message: loc('هل أنت متأكد تماماً؟ سيتم مسح كافة المعاملات والمحافظ نهائياً من هذا الجهاز.', 'Are you sure? All transactions and wallets will be permanently deleted.', 'ഉറപ്പാണോ? ഈ ഫോണിലെ എല്ലാ ഇടപാടുകളും വാലറ്റുകളും ശാശ്വതമായി മായ്ക്കപ്പെടും.'),
      confirmText: loc('مسح نهائي', 'Clear All', 'പൂർണ്ണമായി മായ്ക്കുക'),
      isDestructive: true,
      onConfirm: async () => {
        setConfirmModalState(prev => ({ ...prev, visible: false }));
        try {
        ...
        >
          ...
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}>
              ...
              <Text style={styles.sectionTitle}>{loc('الحساب والمزامنة السحابية', 'Account & Cloud Sync', 'അക്കൗണ്ടും ക്ലൗഡ് സിങ്കും')}</Text>
            </View>
            ...
            {user ? (
              <View style={styles.userCard}>
                <View style={styles.userInfoRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.userSubtext}>
                      {loc('🟢 حساب نشط ومتصل', '🟢 Active & Synced', '🟢 കണക്റ്റഡ് & ആക്ടീവ്')}
                    </Text>
                  </View>
                </View>
                ...
                <View style={styles.userActionsRow}>
                  ...
                  >
                    {syncing ? (
                      ...
                      <>
                        ...
                        <Text style={styles.primaryActionBtnText}>{loc('مزامنة الآن', 'Sync Now', 'ഇപ്പോൾ സിങ്ക് ചെയ്യുക')}</Text>
                      </>
                    )}
                  </Pressable>
                  ...
                  <Pressable onPress={handleLogout} style={({ pressed }) => [styles.secondaryActionBtn, pressed && { opacity: 0.8 }]}>
                    ...
                    <Text style={styles.secondaryActionBtnText}>{loc('خروج', 'Logout', 'ലോഗ് ഔട്ട്')}</Text>
                  </Pressable>
                </View>
              </View>
            ) : (
              <View style={styles.noUserBox}>
                <Text style={styles.noUserText}>
                  {loc('سجل حسابك لحفظ بياناتك ومزامنتها سحابياً بأمان تام', 'Login to secure and sync your data seamlessly', 'ഡാറ്റ സുരക്ഷിതമായി സൂക്ഷിക്കാൻ ലോഗിൻ ചെയ്യുക')}
                </Text>
                <Pressable
                  onPress={() => {
                    safeHaptic.selection();
                ...
                >
                  ...
                  <Text style={styles.primaryActionBtnText}>
                    {loc('تسجيل الدخول / إنشاء حساب', 'Login / Register', 'ലോഗിൻ / രജിസ്റ്റർ')}
                  </Text>
                </Pressable>
              </View>
            )}
          ...
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}>
              ...
              <Text style={styles.sectionTitle}>{loc('المظهر واللغة', 'Appearance & Language', 'രൂപവും ഭാഷയും')}</Text>
            </View>

            {/* Language Selector */}
            <View style={styles.settingBlock}>
              <Text style={styles.settingBlockLabel}>{loc('لغة التطبيق', 'App Language', 'ആപ്പ് ഭാഷ')}</Text>
            ...
            {/* Theme Selector */}
            <View style={styles.settingBlock}>
              <Text style={styles.settingBlockLabel}>{loc('ثيم التطبيق', 'Color Theme', 'വർണ്ണ തീം')}</Text>
              <View style={styles.themeGrid}>
                {[
                  { id: 'light', nameAr: 'نهاري', nameEn: 'Light', nameMl: 'ലൈറ്റ്', icon: 'sunny-outline', primary: '#10B981', bg: '#F8FAFC' },
                  { id: 'dark', nameAr: 'ليلي', nameEn: 'Dark', nameMl: 'ഡാർക്ക്', icon: 'moon-outline', primary: '#10B981', bg: '#090E17' },
                ].map(tItem => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text numberOfLines={1} style={[styles.themeCardText, isActive && styles.themeCardTextActive]}>
                        {loc(tItem.nameAr, tItem.nameEn, tItem.nameMl)}
                      </Text>
                    </Pressable>
                  );
                })}
            ...
            >
              <View style={styles.menuRowLeft}>
                ...
                <Text style={[styles.compactMenuText, { color: '#3B82F6' }]}>
                  {loc('إعداد ودجت الشاشة الرئيسية', 'Home Screen Widgets', 'ഹോം സ്ക്രീൻ വിജറ്റുകൾ')}
                </Text>
              </View>
          ...
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}>
              ...
              <Text style={styles.sectionTitle}>{loc('الأمان والخصوصية', 'Security & Privacy', 'സുരക്ഷയും സ്വകാര്യതയും')}</Text>
            </View>

            <View style={styles.compactSwitchRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.switchLabel}>{loc('قفل رمز PIN', 'PIN Lock', 'പിൻ ലോക്ക്')}</Text>
              </View>
              <Switch
                value={isPinEnabled}
            ...
            <View style={styles.compactSwitchRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.switchLabel}>{loc('بصمة الوجه / الأصبع (Face ID)', 'Face ID / Biometrics', 'ഫേസ് ഐഡി / ബയോമെട്രിക്സ്')}</Text>
              </View>
              <Switch
              ...
              >
                <View style={styles.menuRowLeft}>
                  ...
                  <Text style={styles.compactMenuText}>
                    {loc('سياسة الخصوصية', 'Privacy Policy', 'സ്വകാര്യതാ നയം')}
                  </Text>
                </View>
          ...
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}>
              ...
              <Text style={styles.sectionTitle}>{loc('الإشعارات', 'Notifications', 'അറിയിപ്പുകൾ')}</Text>
            </View>

            <View style={styles.compactSwitchRow}>
              <Text style={styles.switchLabel}>{loc('تذكير مسائي يومي', 'Daily Reminder', 'പ്രതിദിന ഓർമ്മപ്പെടുത്തൽ')}</Text>
              <Switch
            ...
            <View style={styles.compactSwitchRow}>
              <Text style={styles.switchLabel}>{loc('تنبيهات تجاوز الميزانية', 'Budget Alerts', 'ബജറ്റ് മുന്നറിയിപ്പ്')}</Text>
              <Switch
            ...
            <View style={styles.compactSwitchRow}>
              <Text style={styles.switchLabel}>{loc('التقرير الشهري', 'Monthly Digest', 'പ്രതിമാസ റിപ്പോർട്ട്')}</Text>
              <Switch
            ...
            <View style={styles.compactSwitchRow}>
              <Text style={styles.switchLabel}>{loc('الملخص الأسبوعي (كل جمعة)', 'Weekly Digest (Fridays)', 'പ്രതിവാര സംഗ്രഹം (വെള്ളി)')}</Text>
              <Switch
            ...
            <View style={styles.compactSwitchRow}>
              <Text style={styles.switchLabel}>{loc('إنجازات الادخار', 'Savings Milestones', 'സമ്പാദ്യ നേട്ടങ്ങൾ')}</Text>
              <Switch
            ...
            <View style={styles.compactSwitchRow}>
              <Text style={styles.switchLabel}>{loc('تحليل الإنفاق الذكي', 'Smart Spending Insights', 'സ്മാർട്ട് ചെലവ് വിശകലനം')}</Text>
              <Switch
          ...
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}>
              ...
              <Text style={styles.sectionTitle}>{loc('تصدير وإدارة البيانات', 'Data & Export', 'ഡാറ്റയും എക്സ്പോർട്ടും')}</Text>
            </View>
            ...
            >
              <View style={styles.sectionHeader}>
                ...
                <Text style={[styles.sectionTitle, { color: colors.expense }]}>
                  {loc('منطقة الخطر', 'Danger Zone', 'ഡേഞ്ചർ സോൺ')}
                </Text>
              </View>
              <Ionicons
              ...
              <View style={{ gap: 10, marginTop: 4 }}>
                <Text style={styles.dangerSubtext}>
                  {loc(
                    ...
                    'എല്ലാ ഡാറ്റയും മായ്ച്ച് ആപ്പ് പുനഃക്രമീകരിക്കുക.'
                  )}
                </Text>
                <Pressable
                ...
                >
                  ...
                  <Text style={styles.clearBtnText}>
                    {loc('مسح جميع البيانات نهائياً', 'Clear All Data', 'എല്ലാ വിവരങ്ങളും മായ്ക്കുക')}
                  </Text>
                </Pressable>
              </View>
            )}
          ...
          <View style={styles.aboutContainer}>
            <Text style={styles.aboutTitle}>{loc('ميزان - Mizan', 'Mizan App', 'മീസാൻ ആപ്പ്')}</Text>
            <Text style={styles.versionText}>
              {loc('الإصدار 1.0.0', 'Version 1.0.0', 'പതിപ്പ് 1.0.0')}
            </Text>
          </View>
        </ScrollView>
            ...
            <SafeAreaView style={{ flex: 1, width: '100%', maxWidth: 400, justifyContent: 'center', alignItems: 'center', padding: 24 }}>
              ...
              <View style={{ alignItems: 'center', marginBottom: 30, gap: 12 }}>
                ...
                <Text style={{ fontFamily: 'Cairo_700Bold', fontSize: 20, color: colors.text }}>
                  {pinStep === 'enter'
                    ? loc('إنشاء رمز PIN الجديد', 'Create New PIN', 'പുതിയ പിൻ നൽകുക')
                    : loc('تأكيد رمز PIN', 'Confirm PIN Code', 'പിൻ വീണ്ടും നൽകുക')}
                </Text>
                <Text style={{ fontFamily: 'Cairo_600SemiBold', fontSize: 13, color: colors.textSecondary, textAlign: 'center', paddingHorizontal: 20, lineHeight: 18 }}>
                  {pinStep === 'enter'
                    ? loc('أدخل رمز PIN المكون من 4 أرقام لحماية بياناتك:', 'Enter a 4-digit PIN code to secure your data:', 'ഡാറ്റ സുരക്ഷിതമാക്കാൻ 4 അക്ക പിൻ നൽകുക:')
                    : loc('أعد كتابة رمز PIN للتأكيد:', 'Re-enter your PIN code to confirm:', 'സ്ഥിരീകരിക്കാൻ പിൻ വീണ്ടും നൽകുക:')}
                </Text>
              </View>
            ...
            <View style={{ width: '100%', maxWidth: 450, backgroundColor: colors.surface, borderRadius: 24, padding: 20, borderWidth: 1, borderColor: colors.border, gap: 14 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  ...
                  <Text style={{ fontFamily: 'Cairo_700Bold', fontSize: 16, color: colors.text }}>
                    {loc('استعادة النسخة الاحتياطية (JSON)', 'Restore JSON Backup', 'ബാക്കപ്പ് പുനഃസ്ഥാപിക്കുക (JSON)')}
                  </Text>
                </View>
              ...
              <Text style={{ fontFamily: 'Cairo_400Regular', fontSize: 12, color: colors.textSecondary, lineHeight: 18 }}>
                {loc(
                  ...
                  'ഡാറ്റ പുനഃസ്ഥാപിക്കാൻ ബാക്കപ്പ് JSON ഇവിടെ നൽകുക:'
                )}
              </Text>
                  ...
                  {isRestoring ? (
                    ...
                    <>
                      ...
                      <Text style={{ fontFamily: 'Cairo_700Bold', fontSize: 14, color: '#FFF' }}>
                        {loc('تأكيد الاستعادة', 'Confirm Restore', 'പുനഃസ്ഥാപിക്കുക')}
                      </Text>
                    </>
                  )}
                </Pressable>
